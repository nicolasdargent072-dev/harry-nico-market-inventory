...
// Icônes Lucide pour l'esthétique
...
const App = () => {
    ...
    useEffect(() => {
        try {
            ...
            setError("Erreur d'initialisation Firebase. Vérifiez la configuration.");
            setIsLoading(false);
            console.error("Firebase Init Error:", e);
        }
    }, []);

    // Utilitaire pour créer la référence de collection Firestore (CORRIGÉ: 5 segments)
    ...
    useEffect(() => {
        ...
        try {
            ...
            // 2. Dépenses Quotidiennes (on va juste prendre les dernières, simplifiant pour la démo)
            ...
            setError("Erreur lors de la configuration des écouteurs de données.");
        }
    }, [db, userId]);


    // --- Logique d'Application (Fonctions d'écriture Firestore) ---
    ...
    const handleSaveProduct = async (productData) => {
        ...
        try {
            if (productData.id) {
                ...
                alertMessage("Produit mis à jour avec succès!");
            } else {
                await addDoc(productsRef, payload);
                alertMessage("Produit ajouté avec succès!");
            }
        } catch (e) {
    ...
    const handleDeleteProduct = async (productId) => {
        if (!db || !userId || !window.confirm("Êtes-vous sûr de vouloir supprimer ce produit?")) return;

        // CORRECTION: Utilisation de l'utilitaire pour obtenir la référence de document
        const productDocRef = doc(getUserCollection('products'), productId);

        try {
            await deleteDoc(productDocRef);
            alertMessage("Produit supprimé avec succès!");
        } catch (e) {
    ...
    const handleStockTransaction = async (productId, type, quantity) => {
        ...
        // CORRECTION: Utilisation de l'utilitaire pour obtenir les références
        ...
        try {
            ...
            alertMessage(`Transaction de stock (${type}) réussie.`);
        } catch (e) {
    ...
    // Mise à jour des Dépenses Quotidiennes
    const handleUpdateDailyExpenses = async (expenses) => {
        ...
        try {
            ...
            alertMessage("Dépenses quotidiennes enregistrées.");
        } catch (e) {
            console.error("Error updating daily expenses: ", e);
            alertMessage("Erreur lors de la mise à jour des dépenses.", true);
        }
    };
    ...
    const handleUpdateDailyStats = async (stats) => {
        ...
        try {
            ...
            alertMessage("Statistiques du jour enregistrées.");
        } catch (e) {
            console.error("Error updating daily stats: ", e);
            alertMessage("Erreur lors de la mise à jour des statistiques.", true);
        }
    };
    ...
    const calculateDailyMetrics = useMemo(() => {
        ...
        const totalDailyExpenses = (dailyExpenses.thievesLoss || 0) +
            (dailyExpenses.rentCost / 30 || 0) + // Simplification: loyer divisé par 30 jours
            (dailyExpenses.electricityCost / 30 || 0) + // Simplification: électricité divisée par 30 jours
            (dailyExpenses.salaries / 30 || 0) + // Simplification: salaires divisés par 30 jours
            (dailyExpenses.otherCosts || 0);

        // Marge Brute
        const grossMargin = totalDailyRevenue - totalDailyCOGS;
        // Marge Nette (Bénéfice Net)
        const netMargin = grossMargin - totalDailyExpenses;

        // Solde Journalier (pour le bilan)
        const dailyBalance = netMargin; // Le Solde Journalier est le Bénéfice Net (Marge Nette)
    ...
    // --- Composant Modale/Alert personnalisé (pas d'alert()) ---
    ...
    // --- Composants de Vue (Séparés par couleur) ---
        ...
        <div className={`p-8 rounded-xl shadow-lg ${color} h-full`}>
            ...
            <div className="grid md:grid-cols-2 gap-8">
                <div className="bg-white p-6 rounded-lg shadow-md border-t-4 border-blue-500">
                    <h3 className="text-2xl font-semibold text-blue-800 mb-4">Identité du Magasin</h3>
                    ...
                    <p className="text-gray-700 mb-2"><span className="font-medium">Slogan:</span> Le marché qui vous sourit !</p>
                </div>
                <div className="bg-white p-6 rounded-lg shadow-md border-t-4 border-teal-500">
                    <h3 className="text-2xl font-semibold text-teal-800 mb-4">Identité du Gérant</h3>
                    <p className="text-gray-700 mb-2"><span className="font-medium">Nom du Gérant:</span> Monsieur Harry ou Madame Nico</p>
    ...
    const ProductManagementView = ({ color }) => {
        ...
        const [formData, setFormData] = useState({ id: null, name: '', price: 0, cost: 0, stock: 0, emoji: '🥕' });

        useEffect(() => {
            if (editingProduct) {
                setFormData(editingProduct);
            } else {
                setFormData({ id: null, name: '', price: 0, cost: 0, stock: 0, emoji: '🥕' });
            }
        }, [editingProduct]);
            ...
            <div className={`p-8 rounded-xl shadow-lg ${color} h-full overflow-y-auto`}>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md mb-8 border-l-4 border-orange-500">
                    ...
                    <form onSubmit={handleSubmit} className="grid md:grid-cols-3 gap-4">
                        ...
                        <input name="price" type="number" value={formData.price} onChange={handleChange} placeholder="Prix de Vente (€)" required className="p-2 border rounded-md" min="0" step="0.01" />
                        <input name="cost" type="number" value={formData.cost} onChange={handleChange} placeholder="Coût d'Achat (€)" required className="p-2 border rounded-md" min="0" step="0.01" />
                ...
                {/* Liste des Produits (Tableau Coloré) */}
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-orange-200">
                            <thead className="bg-orange-100 text-orange-800 uppercase text-sm">
                                <tr>
                                    <th className="px-6 py-3 text-left font-medium">Émoji</th>
                                    <th className="px-6 py-3 text-left font-medium">Nom</th>
                                    <th className="px-6 py-3 text-left font-medium">Prix Vente (€)</th>
                                    <th className="px-6 py-3 text-left font-medium">Coût Achat (€)</th>
                                    <th className="px-6 py-3 text-left font-medium">Marge Unitaire (€)</th>
                                    ...
                                    return (
                                        <tr key={p.id} className={`hover:bg-orange-50 ${stockColor}`}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap space-x-2">
                                                <button onClick={() => setEditingProduct(p)} className="text-orange-600 hover:text-orange-900 font-medium">Éditer</button>
    ...
    const OrderingView = ({ color }) => {
        ...
        const handleTransaction = (e) => {
            ...
            if (!selectedProduct || quantity <= 0) {
                alertMessage("Veuillez sélectionner un produit et une quantité valide.", true);
                return;
            }
            ...
            <div className={`p-8 rounded-xl shadow-lg ${color} h-full overflow-y-auto`}>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md mb-8 border-l-4 border-purple-500">
                    ...
                    <form onSubmit={handleTransaction} className="grid md:grid-cols-4 gap-4 items-end">
                        ...
                        <input type="number" value={quantity} onChange={(e) => setQuantity(parseInt(e.target.value))} min="1" required placeholder="Quantité" className="p-2 border rounded-md" />
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-lg shadow-md border-t-4 border-green-500">
                        ...
                        <p className="text-3xl font-bold">{totalIn.toLocaleString()} unités</p>
                    </div>
                    <div className="bg-white p-6 rounded-lg shadow-md border-t-4 border-red-500">
                        <h3 className="text-2xl font-semibold text-red-700 mb-2">Bilan des Stocks Sortants</h3>
                        <p className="text-3xl font-bold">{totalOut.toLocaleString()} unités</p>
                    </div>
                </div>

                {/* Historique des Transactions (Tableau Coloré) */}
                <div className="bg-white p-6 rounded-lg shadow-lg">
                    <h3 className="text-xl font-semibold mb-4">Historique Récent des Mouvements de Stock</h3>
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-purple-200">
                            <thead className="bg-purple-100 text-purple-800 uppercase text-sm">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-right font-medium">Quantité</th>
                                ...
                                {stockHistory.map((t) => {
                                    ...
                                    return (
                                        <tr key={t.id} className={`hover:bg-purple-50 ${isIncoming ? 'bg-green-50' : 'bg-red-50'}`}>
                                            <td className={`px-6 py-4 whitespace-nowrap font-bold ${isIncoming ? 'text-green-700' : 'text-red-700'}`}>{isIncoming ? 'Entrée' : 'Sortie'}</td>
    ...
    const ExpensesView = ({ color }) => {
        ...
        // Calcul du total journalier des dépenses (hors coût des produits achetés, qui est COGS)
        ...
        return (
            <div className={`p-8 rounded-xl shadow-lg ${color} h-full overflow-y-auto`}>
                <h2 className="text-3xl font-bold text-rose-900 mb-6 flex items-center">
                    <DollarSign className="w-6 h-6 mr-2" /> Dépense Quotidienne
                </h2>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-rose-500">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        {[
                            { label: "Argents perdu à cause des voleurs (€)", name: "thievesLoss" },
                            { label: "Coûts des produits achetés (Total Annuel/Mois - À des fins de COGS)", name: "productsCostBought" }, // Note: This field is just for display/record, COGS is calculated based on sales
                            { label: "Coût du Loyer (Mensuel - €)", name: "rentCost" },
                            { label: "Coût de l'électricité (Mensuel - €)", name: "electricityCost" },
                            { label: "Salaires des Employées (Mensuel - €)", name: "salaries" },
                            { label: "Autres coûts (Journalier - €)", name: "otherCosts" },
                        ].map(({ label, name }) => (
                        ...
                        <div className="pt-4 border-t border-rose-200">
                            <p className="text-lg font-bold text-rose-800">Total des Frais Opérationnels Journaliers Estimés (hors COGS): {totalDailyExpense} €</p>
                        </div>
    ...
    const FinancialView = ({ color }) => {
        ...
        // Ici on simule l'agrégation en utilisant les données journalières actuelles comme base.

        const metrics = calculateDailyMetrics;
        const isPositive = parseFloat(metrics.dailyBalance) >= 0;

        const dataPoints = [
            { label: "Solde Journalier (Bénéfice Net)", value: metrics.dailyBalance + ' €', color: isPositive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' },
            { label: "Bilan Mensuel (Estimé)", value: (metrics.dailyBalance * 30).toFixed(2) + ' €', color: 'bg-indigo-100 text-indigo-800' },
            { label: "Bilan Trimestriel (Estimé)", value: (metrics.dailyBalance * 90).toFixed(2) + ' €', color: 'bg-purple-100 text-purple-800' },
            { label: "Bilan Semestriel (Estimé)", value: (metrics.dailyBalance * 180).toFixed(2) + ' €', color: 'bg-pink-100 text-pink-800' },
            { label: "Bilan Annuel (Estimé)", value: (metrics.dailyBalance * 365).toFixed(2) + ' €', color: 'bg-teal-100 text-teal-800' },
        ];

        // Bilan des stocks (réutilisation de la logique de OrderingView)
        const { totalIn, totalOut } = useMemo(() => {
            // CORRECTION: Récupération de la référence de collection
        ...
        return (
            <div className={`p-8 rounded-xl shadow-lg ${color} h-full overflow-y-auto`}>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md border-t-4 border-cyan-500">
                    <h3 className="text-2xl font-semibold text-cyan-800 mb-4">Bilan des Stocks (Total depuis le début)</h3>
                    <div className="flex justify-around space-x-4">
                        ...
                        <div className="text-center p-4 bg-red-50 rounded-lg flex-1">
                            ...
                            <p className="text-xs text-gray-500">unités vendues/perdues</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        return (
            <div className={`p-8 rounded-xl shadow-lg ${color} h-full overflow-y-auto`}>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md mb-8 border-l-4 border-amber-500">
                    <h3 className="text-xl font-semibold mb-4">Saisie des Données pour le <span className="text-amber-600">{dailyStats.date}</span></h3>
                    <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-4">
                        {[
                            { label: "Produits non trouvés (Plaintes)", name: "notFounds", icon: <Frown className="w-4 h-4 text-gray-500" /> },
                            ...
                            { label: "Plaintes concernant les saletés", name: "dirtComplaints", icon: <Package className="w-4 h-4 text-gray-500" /> },
                            // Champs nécessaires pour l'Analyse de Rentabilité (visibles pour la saisie)
                            { label: "Total des Ventes (Chiffre d'Affaires)", name: "dailySales", step: "0.01", min: "0" },
                            { label: "Coût Total des Produits Vendus (COGS)", name: "productsCostTotal", step: "0.01", min: "0" },
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg">
                    <h3 className="text-xl font-semibold mb-4">Synthèse des Problèmes ({dailyStats.date})</h3>
                    {isDataLoaded ? (
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-amber-200">
                                <thead className="bg-amber-100 text-amber-800 uppercase text-sm">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 text-left font-medium">Interprétation</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-amber-100">
                                    <tr className="hover:bg-amber-50">
                                        <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">Produits non trouvés</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-lg font-bold">{dailyStats.notFounds}</td>
                                        <td className="px-6 py-4 whitespace-nowrap">{dailyStats.notFounds > 5 ? "Urgence stock !" : "Peut-être dû au réassort."}</td>
                                    </tr>
                                    <tr className="hover:bg-amber-50">
                                    ...
                                    <tr className="hover:bg-amber-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">{dailyStats.thefts > 0 ? "Renforcer la sécurité." : "Bonne journée."}</td>
                                    </tr>
                                    <tr className="hover:bg-amber-50">
                                        <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">Plaintes : Saletés</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-lg font-bold">{dailyStats.dirtComplaints}</td>
                                        <td className="px-6 py-4 whitespace-nowrap">{dailyStats.dirtComplaints > 0 ? "Faire un ménage approfondi." : "Propreté satisfaisante."}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    ) : (
                        <p className="text-gray-500">Aucune donnée statistique enregistrée pour aujourd'hui.</p>
                    )}
                </div>
            </div>
        );
    ...
    // 7. Analyse de Rentabilité Journalière
        ...
        return (
            <div className={`p-8 rounded-xl shadow-lg ${color} h-full`}>
                <h2 className="text-3xl font-bold text-emerald-900 mb-6 flex items-center">
                    <TrendingUp className="w-6 h-6 mr-2" /> Analyse de Rentabilité Journalière
                </h2>

                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    {/* Colonne 1: Ventes et Coûts */}
                    <div className="bg-white p-6 rounded-lg shadow-md border-t-4 border-emerald-500">
                        <h3 className="text-xl font-semibold text-emerald-800 mb-4">Ventes & Coûts</h3>
                        <p className="text-lg mb-2"><span className="font-medium">Chiffre d'Affaires (CA):</span> <span className="font-bold text-2xl text-blue-600">{metrics.totalDailyRevenue} €</span></p>
                        <p className="text-lg mb-2"><span className="font-medium">Coût des Marchandises Vendues (COGS):</span> <span className="font-bold text-2xl text-red-600">{metrics.totalDailyCOGS} €</span></p>
                        <p className="text-lg mb-2"><span className="font-medium">Frais Opérationnels Journaliers:</span> <span className="font-bold text-2xl text-red-600">{metrics.totalDailyExpenses} €</span></p>
                    </div>
                    ...
                    <div className={`p-6 rounded-lg shadow-md border-t-4 ${isGrossPositive ? 'bg-green-50 border-green-500' : 'bg-red-50 border-red-500'}`}>
                        ...
                        <p className="text-4xl font-extrabold mt-1">{metrics.grossMargin} €</p>
                        <p className={`mt-2 ${isGrossPositive ? 'text-green-700' : 'text-red-700'} font-medium`}>
                            {isGrossPositive ? "Votre marge est positive avant les frais fixes !" : "Attention: Le coût des produits vendus dépasse vos revenus."}
                        </p>
                    </div>
                    ...
                    <div className={`p-6 rounded-lg shadow-md border-t-4 ${isNetPositive ? 'bg-emerald-100 border-emerald-500' : 'bg-pink-100 border-pink-500'}`}>
                        <h3 className="text-xl font-semibold text-gray-800 mb-4">Marge Nette (Bénéfice Net)</h3>
                        <p className="text-4xl font-extrabold mt-1">{metrics.netMargin} €</p>
                        <p className={`mt-2 ${isNetPositive ? 'text-emerald-700' : 'text-pink-700'} font-medium`}>
                            {isNetPositive ? "Félicitations, vous êtes rentable pour la journée." : "Action requise: les dépenses dépassent les profits bruts."}
                        </p>
                    </div>
                </div>

                <p className="text-sm text-gray-600 mt-4 p-4 bg-gray-100 rounded-md">
                    *L'analyse est basée sur les données enregistrées aujourd'hui. Assurez-vous d'entrer votre Chiffre d'Affaires et Coût des Marchandises Vendues dans l'espace "Statistique du Jour" et les frais fixes dans "Dépense Quotidienne".
                </p>
            </div>
        );
    };

    // 8. Téléchargement des Bilans (Simulé)
    const ExportView = ({ color }) => {
        const handleDownload = (format) => {
            alertMessage(`Téléchargement du Bilan au format ${format} simulé. (Fonctionnalité complète non disponible dans cette démo en un seul fichier)`);
        };

        return (
            <div className={`p-8 rounded-xl shadow-lg ${color} h-full`}>
                <h2 className="text-3xl font-bold text-gray-900 mb-6 flex items-center">
                    <Download className="w-6 h-6 mr-2" /> Téléchargement des Bilans
                </h2>

                <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-gray-500">
                    <p className="text-lg mb-6">Sélectionnez le format pour télécharger l'ensemble des bilans agrégés (Journaliers, Mensuels, Stocks, etc.).</p>
    ...
    const NavItem = ({ view, icon: Icon }) => (
        <button
            ...
            className={`flex items-center p-3 rounded-lg w-full text-left transition ${currentView === view
        ...
        <div className="flex h-screen w-full bg-gray-100 font-inter">
            ...
            <div className="bg-blue-900 w-64 flex flex-col p-4 shadow-2xl">
                ...
                <nav className="flex flex-col space-y-2 flex-grow">
                    ...
                    <p className="text-blue-400 text-sm font-semibold uppercase px-2 mb-1">Opérations</p>
